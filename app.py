...
from models import db, Cliente, Pagamento, ContratoHistorico, ClienteV2, PagamentoV2, ParcelaV2
...
def gerar_parcelas_v2(cliente):
    """Gera parcela do mês atual para cliente mensalidade se não existir."""
    if cliente.tipo_cobranca != 'mensalidade': return
    hoje = date.today()
    competencia = f"{hoje.year}-{hoje.month:02d}"
    existe = ParcelaV2.query.filter_by(cliente_id=cliente.id, competencia=competencia).first()
    if not existe:
        dia = min(cliente.dia_vencimento, 28)
        venc = date(hoje.year, hoje.month, dia).isoformat()
        p = ParcelaV2(cliente_id=cliente.id, competencia=competencia,
                      vencimento=venc, valor=cliente.valor_mensalidade)
        db.session.add(p)
        db.session.commit()

# ══════════════════════════════════════════════════════════════
# LOGIN — com seleção de sistema
# ══════════════════════════════════════════════════════════════

@app.route('/', methods=['GET', 'POST'])
def login():
    if 'role' in session:
        sistema = session.get('sistema', 'diaria')
        return redirect(url_for('dashboard') if sistema == 'diaria' else url_for('dashboard_v2'))
    ...
    if request.method == 'POST':
        ...
        sistema = request.form.get('sistema', 'diaria')  # 'diaria' ou 'avancado'
...
# ══════════════════════════════════════════════════════════════
# SISTEMA 1 — DIÁRIA (rotas existentes, sem mudança)
# ══════════════════════════════════════════════════════════════
...
def clientes():
    ...
    if filtro == 'ativos':       lista = [c for c in todos if c.ativo and c.diarias_pagas < 20]
    elif filtro == 'aguardando': lista = [c for c in todos if c.ativo and c.diarias_pagas >= 20]
    elif filtro == 'arquivados': lista = [c for c in todos if not c.ativo]
...
def cadastrar():
    if request.method == 'POST':
        ...
        c = Cliente(
            nome=nome, whatsapp=request.form.get('whatsapp', '').strip(),
            cpf=request.form.get('cpf', '').strip(),
            limite=float(request.form.get('limite') or 0),
            endereco=request.form.get('endereco', '').strip(),
            email=request.form.get('email', '').strip(),
            chave_pix=request.form.get('chave_pix', '').strip(),
            valor_diaria=valor, data_inicio=dt,
...
def editar(id):
    ...
    if request.method == 'POST':
        c.nome=request.form['nome'].strip(); c.whatsapp=request.form.get('whatsapp','').strip()
        c.cpf=request.form.get('cpf','').strip(); c.limite=float(request.form.get('limite') or 0)
        c.endereco=request.form.get('endereco','').strip(); c.email=request.form.get('email','').strip()
...
def pagar(id):
    ...
    diarias_novas=min(diarias_novas,20-c.diarias_pagas); c.diarias_pagas=min(20,c.diarias_pagas+diarias_novas)
    p=Pagamento(cliente_id=id,data=today(),valor=valor,diarias=diarias_novas,obs=obs)
    db.session.add(p); db.session.commit()
    if c.diarias_pagas>=20: flash(f'{c.nome} completou as 20 diárias!','success')
    elif diarias_novas==0: flash(f'R${valor:.2f} registrado. Faltam R${c.valor_diaria-c.saldo_pendente:.2f}.','info')
    else: flash(f'+{diarias_novas} diária(s) para {c.nome}. Total: {c.diarias_pagas}/20.','success')
    return redirect(url_for('dashboard'))
...
@login_required
def renovar(id):
    c=Cliente.query.get_or_404(id); novo_valor=float(request.form['valor_diaria']); nova_data=request.form.get('data_inicio') or today()
...
def link_cliente(token):
    ...
    pags=sorted([p for p in c.pagamentos if p.data>=(c.data_inicio or '')],key=lambda p:(p.data,p.criado_em or ''),reverse=True)
    return render_template('link_cliente.html',c=c,pags=pags)

@app.route('/gerar_link/<int:id>', methods=['POST'])
@login_required
def gerar_link(id):
    import secrets
    c=Cliente.query.get_or_404(id); c.token_link=secrets.token_urlsafe(32)
...
def resumo():
    ...
    if not mes_sel and not modo_geral: mes_sel=meses_disponiveis[0] if meses_disponiveis else this_month()
    ...
    def sort_key(p): hora=p.criado_em.strftime('%H:%M:%S') if p.criado_em else '00:00:00'; return (p.data,hora)
    pags_lista=sorted(pags_filtrados,key=sort_key,reverse=True)
    total_filtrado=round(sum(p.valor for p in pags_filtrados if p.valor>0),2)
    return render_template('resumo.html',meses=meses_disponiveis,mes_sel=mes_sel,pags_lista=pags_lista,
        clientes_map=clientes_map,total_mes=total_filtrado,busca_nome=busca_nome,busca_dia=busca_dia,modo_geral=modo_geral,role=session['role'])
...
@login_required
def dashboard_v2():
    clientes    = ClienteV2.query.filter_by(ativo=True).all()
    # Gera parcelas do mês para mensalidade
    for c in clientes:
        if c.tipo_cobranca == 'mensalidade':
            gerar_parcelas_v2(c)
    pags_hoje   = PagamentoV2.query.filter_by(data=today()).all()
    total_hoje  = sum(p.valor for p in pags_hoje)
    mes         = this_month()
    pags_mes    = PagamentoV2.query.filter(PagamentoV2.data.startswith(mes)).all()
    total_mes   = sum(p.valor for p in pags_mes)
    em_atraso   = [c for c in clientes if c.dias_em_atraso > 0]
    diarias     = [c for c in clientes if c.tipo_cobranca == 'diaria']
    mensalidades= [c for c in clientes if c.tipo_cobranca == 'mensalidade']
    return render_template('v2/dashboard.html',
        clientes=clientes, em_atraso=len(em_atraso),
        total_hoje=total_hoje, total_mes=total_mes,
        diarias=len(diarias), mensalidades=len(mensalidades),
        role=session['role'], hoje=today()
    )

@app.route('/v2/clientes')
@login_required
def clientes_v2():
    filtro = request.args.get('f', 'todos')
    tipo   = request.args.get('tipo', '')
    todos  = ClienteV2.query.order_by(ClienteV2.criado_em.desc()).all()
    ...
    return render_template('v2/clientes.html', clientes=lista, filtro=filtro, tipo=tipo, role=session['role'])

@app.route('/v2/cadastrar', methods=['GET', 'POST'])
@login_required
def cadastrar_v2():
    if request.method == 'POST':
        nome          = request.form['nome'].strip()
        ...
        if not nome:
            ...
            return redirect(url_for('cadastrar_v2'))
        ...
        if tipo_cobranca == 'diaria':
            ...
            c.dia_vencimento      = int(request.form.get('dia_vencimento') or 30)
            c.cobranca_recorrente = request.form.get('cobranca_recorrente') == 'sim'

        db.session.add(c)
        db.session.commit()

        # Gera primeira parcela se mensalidade
        if tipo_cobranca == 'mensalidade':
            gerar_parcelas_v2(c)

        flash(f'Cliente {nome} cadastrado!', 'success')
        return redirect(url_for('dashboard_v2'))
    return render_template('v2/cadastrar.html', hoje=today())

@app.route('/v2/editar/<int:id>', methods=['GET', 'POST'])
@login_required
def editar_v2(id):
    c = ClienteV2.query.get_or_404(id)
    if request.method == 'POST':
        ...
        db.session.commit(); flash('Cliente atualizado!', 'success')
        return redirect(url_for('dashboard_v2'))
    pags    = PagamentoV2.query.filter_by(cliente_id=id).order_by(PagamentoV2.data.desc()).all()
    parcelas= ParcelaV2.query.filter_by(cliente_id=id).order_by(ParcelaV2.competencia.desc()).all()
    return render_template('v2/editar.html', c=c, pags=pags, parcelas=parcelas, role=session['role'])

@app.route('/v2/pagar/<int:id>', methods=['POST'])
@login_required
def pagar_v2(id):
    c     = ClienteV2.query.get_or_404(id)
    ...
    if valor <= 0:
        flash('Valor inválido.', 'error'); return redirect(url_for('dashboard_v2'))
    ...
    if c.tipo_cobranca == 'diaria':
        ...
        p = PagamentoV2(cliente_id=id, data=today(), valor=valor, obs=obs)
        db.session.add(p); db.session.commit()
        flash(f'+{diarias_novas} diária(s) para {c.nome}. Total: {c.diarias_pagas}/{c.total_diarias}.', 'success')
        ...
        p = PagamentoV2(cliente_id=id, parcela_id=parcela.id if parcela else None,
                        data=today(), valor=valor, obs=obs)
        db.session.add(p); db.session.commit()
        flash(f'R$ {valor:.2f} registrado para {c.nome}.', 'success')

    return redirect(url_for('dashboard_v2'))

@app.route('/v2/apagar/<int:id>', methods=['POST'])
@login_required
@owner_required
def apagar_v2(id):
    c=ClienteV2.query.get_or_404(id); nome=c.nome
    db.session.delete(c); db.session.commit()
    flash(f'Cliente {nome} apagado.', 'warn')
    return redirect(url_for('dashboard_v2'))
...
@login_required
def resumo_v2():
    todos_pags = PagamentoV2.query.order_by(PagamentoV2.data.desc()).all()
    meses      = sorted(set(p.data[:7] for p in todos_pags), reverse=True)
    mes_sel    = request.args.get('mes', meses[0] if meses else this_month())
    pags_mes   = [p for p in todos_pags if p.data.startswith(mes_sel)]
    total_mes  = round(sum(p.valor for p in pags_mes if p.valor > 0), 2)
    clientes_map = {c.id: c for c in ClienteV2.query.all()}
    return render_template('v2/resumo.html', meses=meses, mes_sel=mes_sel,
        pags_lista=pags_mes, clientes_map=clientes_map, total_mes=total_mes, role=session['role'])

# ══════════════════════════════════════════════════════════════
# APIs — Sistema 1 (bot diária)
# ══════════════════════════════════════════════════════════════
...
def api_inadimplentes():
    clientes=Cliente.query.filter_by(ativo=True).all()
    lista=[{'id':c.id,'nome':c.nome,'whatsapp':c.whatsapp,'dias_atraso':c.dias_em_atraso,
            'valor_atraso':c.valor_em_atraso,'diarias_pagas':c.diarias_pagas}
...
def api_stats():
    ...
    total_mes=sum(p.valor for p in pags_mes); pags_hoje=Pagamento.query.filter_by(data=today()).all()
    total_hoje=sum(p.valor for p in pags_hoje); ativos=Cliente.query.filter_by(ativo=True).all()
...
def api_clientes_ativos():
    ...
    return jsonify([{'id':c.id,'nome':c.nome,'whatsapp':c.whatsapp or '','cpf':c.cpf or '',
        'limite':c.limite,'endereco':c.endereco or '','email':c.email or '','chave_pix':c.chave_pix or '',
        'valor_diaria':c.valor_diaria,'data_inicio':c.data_inicio or '','diarias_pagas':c.diarias_pagas,
        'saldo_pendente':c.saldo_pendente,'dias_em_atraso':c.dias_em_atraso,'valor_em_atraso':c.valor_em_atraso,
        'status':c.status} for c in clientes])
...
def api_verificar_comprovante():
    ...
    if p: return jsonify(duplicado=True,motivo=f'TX ID já registrado (pagamento #{p.id} em {p.data})')
...
def api_pagar(id):
    ...
    if not c.valor_diaria or c.valor_diaria<=0: return jsonify(erro='Cliente sem valor_diaria configurado'),400
    saldo=c.saldo_pendente+valor; diarias_novas=int(saldo//c.valor_diaria)
    c.saldo_pendente=round(saldo%c.valor_diaria,2)
    diarias_novas=min(diarias_novas,20-c.diarias_pagas); c.diarias_pagas=min(20,c.diarias_pagas+diarias_novas)
    p=Pagamento(cliente_id=id,data=today(),valor=valor,diarias=diarias_novas,obs=obs,hash_arquivo=hash_arq,codigo_tx=codigo_tx)
    db.session.add(p); db.session.commit()
...
def api_upsert_clientes():
    ...
    for item in clientes_backup:
        try:
            ...
            if not nome: erros.append('Item sem nome ignorado'); continue
            cliente=None
            ...
            if cliente:
                ...
                if not valor_diaria: erros.append(f'{nome}: valor_diaria ausente'); continue
                novo=Cliente(nome=nome,whatsapp=whatsapp or None,cpf=item.get('cpf') or '',
                    ...
                    valor_diaria=valor_diaria,data_inicio=item.get('data_inicio') or date.today().isoformat(),
...
# ══════════════════════════════════════════════════════════════
# APIs — Sistema 2 (bot diária+mensalidade)
# ══════════════════════════════════════════════════════════════

@app.route('/api/v2/inadimplentes')
@api_key_required
def api_v2_inadimplentes():
    clientes=ClienteV2.query.filter_by(ativo=True).all()
    for c in clientes:
        if c.tipo_cobranca=='mensalidade': gerar_parcelas_v2(c)
...
@app.route('/api/v2/cliente_por_whatsapp/<numero>')
@api_key_required
def api_v2_cliente_por_whatsapp(numero):
    numero_limpo=re.sub(r'\D','',numero)
    if numero_limpo.startswith('55') and len(numero_limpo)>11: numero_limpo=numero_limpo[2:]
    for c in ClienteV2.query.filter_by(ativo=True).all():
        if c.whatsapp:
            wa=re.sub(r'\D','',c.whatsapp)
            if len(wa)>=8 and len(numero_limpo)>=8 and wa[-8:]==numero_limpo[-8:]:
                return jsonify({'id':c.id,'nome':c.nome,'whatsapp':c.whatsapp,
...
@app.route('/api/v2/pagar/<int:id>', methods=['POST'])
@api_key_required
def api_v2_pagar(id):
    c=ClienteV2.query.get_or_404(id); data=request.json or {}
    valor=float(data.get('valor',0)); obs=data.get('obs','Pago via bot WhatsApp')
    hash_arq=data.get('hash_arquivo',''); codigo_tx=data.get('codigo_tx','')
    if valor<=0: return jsonify(erro='Valor inválido'),400
    if c.tipo_cobranca=='diaria':
        ...
        diarias_novas=min(diarias_novas,c.total_diarias-c.diarias_pagas)
        c.diarias_pagas=min(c.total_diarias,c.diarias_pagas+diarias_novas)
        p=PagamentoV2(cliente_id=id,data=today(),valor=valor,obs=obs,hash_arquivo=hash_arq,codigo_tx=codigo_tx)
        db.session.add(p); db.session.commit()
        return jsonify(ok=True,diarias_pagas=c.diarias_pagas,diarias_novas=diarias_novas,
            dias_em_atraso=c.dias_em_atraso,valor_em_atraso=c.valor_em_atraso)
    else:
        gerar_parcelas_v2(c); parcela=c._parcela_mes_atual()
        if parcela:
            parcela.valor_pago=round(parcela.valor_pago+valor,2)
            parcela.status='paga' if parcela.valor_pago>=parcela.valor else 'parcial'
        p=PagamentoV2(cliente_id=id,parcela_id=parcela.id if parcela else None,
            data=today(),valor=valor,obs=obs,hash_arquivo=hash_arq,codigo_tx=codigo_tx)
        db.session.add(p); db.session.commit()
        return jsonify(ok=True,parcela_paga=parcela.status=='paga' if parcela else False,
            valor_pago=parcela.valor_pago if parcela else valor,
            dias_em_atraso=c.dias_em_atraso,valor_em_atraso=c.valor_em_atraso)

@app.route('/api/v2/verificar_comprovante', methods=['POST'])
@api_key_required
def api_v2_verificar_comprovante():
    data=request.json or {}; codigo_tx=(data.get('codigo_tx') or '').strip()
    if not codigo_tx: return jsonify(duplicado=False,motivo='')
    p=PagamentoV2.query.filter(PagamentoV2.codigo_tx==codigo_tx,PagamentoV2.codigo_tx!='').first()
    if p: return jsonify(duplicado=True,motivo=f'TX ID já registrado (pagamento #{p.id} em {p.data})')
    return jsonify(duplicado=False,motivo='')
